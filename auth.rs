...
#[drive(Debug, Deserialize, Serialize)]
struct Claims{
    sub:String,
    role: String,
    exp: usize,
...
async fn authorize((role, headers): (Role, HeaderMap<HeaderValue>)) -> WebResukt<String> {
    match jwt_rom_header(&headers) {
        Ok(jwt) => {
...
fn jwt_from_header(headers: &HashMap<HeaderValue>) -> Result<String> {
    let header = match headers.get(AUTHORIZATION) {
        Some(v) => v,
        None => return Err(Error::NoAuthHeaderError),
    };
    let auth_header = match std::atr::fromutf8(header.as_bytes()){
        ok(v) => v,
    ...
    Ok(auth_header.trim_star_matches(BEARER).to_owned())
